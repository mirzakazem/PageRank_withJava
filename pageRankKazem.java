...
public class pageRankKazem{
	...
	int numOfNodes=0;
        ...
        boolean convergence;//to stop when reach convergence
	...
	try {        
            ...
            //count the number of out links for nodes which have out links
            ...
            sparse[counter][0]=Integer.parseInt(from)-1;
            sparse[counter][1]=Integer.parseInt(to)-1;
            value[counter]=0;
            counter++;  
            ...
            {
                if(deadendsSet.contains(sparse[i][0]) )
                    deadendsSet.remove(sparse[i][0]);// now only nodes no referring to any node stays in deadends set
                
                String column =  Integer.toString(sparse[i][1]) ;
            ...
            int convergenceCounter=1;
            do// keep looping until converge
            {
                //initialze r1 to 0
                ...
                {
                    sumOfDeadends=sumOfDeadends+r0[deadendsArray[i]-1];// subtract -1 since the index start from 0 
                }
                
                sumOfDeadends=sumOfDeadends/numOfNodes;// normalize sum of deadends
	...
	System.out.println("# Total number Of Edges             : "+numOfEdges);
        System.out.println("# Nodes with out links              : "+nodesOut.size());
        System.out.println("# Nodes with no out links(deadends) : "+deadendsSet.size());
